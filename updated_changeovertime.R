...
enroll2014<-mydata[mydata$EntryDate<=as.Date('12-31-2014',format="%m-%d-%Y")&is.na(mydata$ExitDate),]
length(unique(enroll2014$PersonalID))#110


enroll2014<-mydata[mydata$EntryDate<=as.Date('12-31-2014',format="%m-%d-%Y")
                   & (mydata$ExitDate>=as.Date('01-01-2014',format="%m-%d-%Y")
                      |is.na(mydata$ExitDate)),]
length(unique(enroll2014$PersonalID))


enroll2015<-mydata[mydata$EntryDate<=as.Date('12-31-2015',format="%m-%d-%Y") 
                   & (mydata$ExitDate>=as.Date('01-01-2015',format="%m-%d-%Y")
                   |is.na(mydata$ExitDate)),]
length(unique(enroll2015$PersonalID))


enroll2016<-mydata[mydata$EntryDate<=as.Date('12-31-2016',format="%m-%d-%Y") 
                   & (mydata$ExitDate>=as.Date('01-01-2016',format="%m-%d-%Y")
                   |is.na(mydata$ExitDate)),]
length(unique(enroll2016$PersonalID))



enroll2017<-mydata[mydata$EntryDate<=as.Date('12-31-2017',format="%m-%d-%Y") 
                   & (mydata$ExitDate>=as.Date('01-01-2017',format="%m-%d-%Y")
                      |is.na(mydata$ExitDate)),]
length(unique(enroll2017$PersonalID))

enroll2018<-mydata[mydata$EntryDate<=as.Date('12-31-2018',format="%m-%d-%Y") 
                   & (mydata$ExitDate>=as.Date('01-01-2018',format="%m-%d-%Y")
                      |is.na(mydata$ExitDate)),]
